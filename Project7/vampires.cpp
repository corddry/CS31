...
void Arena::display(string msg) const
{
	...
	for (r = 1; r <= rows(); r++) {
		for (c = 1; c <= cols(); c++) {
			...
			int vamps = numberOfVampiresAt(r, c);
			if (vamps == 0) {
				continue;
			} else if (vamps == 1) {
				displayGrid[r - 1][c - 1] = 'V';
			} else if (vamps <= 8 && vamps >= 2) {
				displayGrid[r - 1][c - 1] = '0' + vamps;			//check this works
			} else if (vamps >= 9) {
...
bool Arena::addVampire(int r, int c)
{
	...
	if (m_nVampires >= MAXVAMPIRES) {
		...
		m_vampires[m_totVampires] = new Vampire(this, r, c);
		m_totVampires++;
		m_nVampires++;
	}
...
int dangerousVamps(const Arena& a, int r, int c) {
	int count = 0;
	if (a.numberOfVampiresAt(r, c) != 0) {
		count = 99;
	}
	count += a.numberOfVampiresAt(r + 1, c);
	count += a.numberOfVampiresAt(r - 1, c);
	count += a.numberOfVampiresAt(r, c - 1);
	count += a.numberOfVampiresAt(r, c + 1);
	return count;
}
...
bool recommendMove(const Arena& a, int r, int c, int& bestDir)
{
	int least = dangerousVamps(a, r, c);
	int best = -1;
	if (dangerousVamps(a, r + 1, c) < least  && (r + 1 <= a.rows())) {
		best = SOUTH;
		least = dangerousVamps(a, r + 1, c);
	}
	if (dangerousVamps(a, r - 1, c) < least && (r - 1 >= 1)) {
		best = NORTH;
		least = dangerousVamps(a, r - 1, c);
	}
	if (dangerousVamps(a, r, c + 1) < least && (c + 1 <= a.cols())) {
		best = EAST;
		least = dangerousVamps(a, r, c + 1);
	}
	if (dangerousVamps(a, r, c - 1) < least && (c - 1 >= 1)) {
		best = WEST;
		least = dangerousVamps(a, r, c - 1);
	}
	if (best == -1) {
